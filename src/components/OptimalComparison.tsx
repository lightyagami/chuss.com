...
export const OptimalComparison: React.FC<Props> = ({
  ...
  if (!currentMove) {
    return (
      <div className="bg-white dark:bg-black border border-slate-200 dark:border-zinc-800 rounded-lg p-5 text-center text-slate-600 dark:text-zinc-400 flex flex-col items-center gap-2 shadow-xs transition-colors">
        <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 flex items-center justify-center text-slate-700 dark:text-zinc-300">
        ...
        <p className="text-xs text-slate-500 dark:text-zinc-400 max-w-xs leading-relaxed">
          Use the playback controls or press the <kbd className="px-1.5 py-0.5 bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded text-[10px] text-slate-700 dark:text-zinc-300">Right Arrow</kbd> to step through the game and view move-by-move engine analysis.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white dark:bg-black border border-slate-200 dark:border-zinc-800 rounded-lg p-4 shadow-xs flex flex-col gap-4 transition-colors">
      ...
      {showWhatIf && (
        <WhatIfExplorer fen={currentMove.fenBefore} />
      )}
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 pt-1 border-t border-slate-100 dark:border-zinc-800 text-xs">
        <div className="flex items-center gap-1.5 flex-wrap">
          <button
            ...
            className={`flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-md border transition ${
              showBestMoveArrow
          ...
          <button
            ...
            className={`flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-md border transition ${
              showWhatIf
                ? 'bg-sky-50 dark:bg-sky-950/50 border-sky-300 dark:border-sky-500 text-sky-800 dark:text-sky-300 font-medium'
                : 'bg-white dark:bg-zinc-950 hover:bg-slate-50 dark:hover:bg-zinc-900 border-slate-300 dark:border-zinc-700 text-slate-600 dark:text-zinc-300'
            }`}
          >
            <GitBranch size={13} />
            <span>What-If ({showWhatIf ? 'Hide' : 'Explore'})</span>
          </button>

          {onExportCardImage && (
            <button
              onClick={onExportCardImage}
              className="flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-md bg-white dark:bg-zinc-950 hover:bg-slate-50 dark:hover:bg-zinc-900 border border-slate-300 dark:border-zinc-700 text-slate-600 dark:text-zinc-300 transition"
              title="Export position and eval as shareable PNG image"
            >
              <Camera size={13} />
        ...
        >
          <Columns2 size={14} />
          <span>{showDualBoard ? 'Close Dual' : 'Side-by-Side'}</span>
        </button>
      </div>
    </div>
  );
